...
manager = DB_Manager(DATABASE)  # Перенес создание менеджера сюда
...
def info_project(message, user_id, project_name):
    ...
    if info:  # Добавил проверку что проект найден
        info = info[0]
...
def name_project(message):
    if message.text == cancel_button:  # Добавил проверку отмены
        cansel(message)
        return
    name = message.text
    ...
    bot.register_next_step_handler(message, desc_project, data=data)

def desc_project(message, data):
    if message.text == cancel_button:  # Добавил проверку отмены
        cansel(message)
        return
...
def text_handler(message):
    user_id = message.from_user.id
    projects = manager.get_projects(user_id)  # Исправил получение проектов
    if projects:
